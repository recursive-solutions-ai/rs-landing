	...
	it('no page or component contains the `/${locale}` href antipattern', () => {
		...
		for (const dir of SCAN_DIRS) {
			for (const file of walk(dir)) {
				if (readFileSync(file, 'utf8').includes(ANTIPATTERN)) offenders.push(file.slice(SRC.length + 1))
			}
		}
		expect(offenders, `Use localizedPath(path, locale) instead of \`/\${locale}/...\` in: ${offenders.join(', ')}`).toEqual([])
	})
})
