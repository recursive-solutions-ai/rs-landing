...
- **Stages** — the **five** showcase steps `01–05` use the **pillar names** exactly (tab labels + in-screen markers + the per-stage caption that sits above the device frame): `01` **Attract** · `02` **Engage** · `03` **Capture** · `04` **Convert** · `05` **Optimize**. Stage `01` Attract **reuses the Attract pillar's own animation** — `AttractVisual` rendered `bare` inside the showcase device frame (paste URL → fetched site → six brand-voice redesigns); in bare mode its marker reads `01 / attract` and the `System · 01` label is hidden. Stages `02–05` are `LucyAnimation`'s four scenes (markers `02 / engage` … `05 / optimize`); the showcase passes `active − 1` to it (tab 1 → scene 0, …).
