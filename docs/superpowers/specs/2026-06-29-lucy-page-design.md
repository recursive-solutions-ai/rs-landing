...
- **Stages** — the **five** showcase steps `01–05`, aligned 1:1 with the five pillars (the pillars are the detailed view of each "look inside Lucy" stage): `01` Attract → **Be Found**, `02` Engage → **Content Engine**, `03` Capture → **Lead Capture**, `04` Convert → **Pipeline**, `05` Optimize → **Custom Experts**. Stage `01` Be Found **reuses the Attract pillar's own animation** — `AttractVisual` rendered `bare` inside the showcase device frame (paste URL → fetched site → six brand-voice redesigns) — so the showcase stage and pillar 01 are literally the same animation. Stages `02–05` are `LucyAnimation`'s four scenes; the showcase passes `active − 1` to it (tab 1 → scene 0, …). `LucyAnimation` keeps its native markers `02–05`.
